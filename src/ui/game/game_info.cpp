...
void GameInfo::DrawGameOverAndLeaderboard()
{
    ...
    int totalPlayerCount = playerManager->GetTotalPlayerCount();
    ...
    std::sort(players.begin(), players.end(), [](const std::shared_ptr<Player>& p1, const std::shared_ptr<Player>& p2){
        return p1->GetDeactivatedTurn() > p2->GetDeactivatedTurn();
    });
